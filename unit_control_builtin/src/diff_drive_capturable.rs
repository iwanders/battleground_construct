...
impl UnitControl for DiffDriveCapturable {
    fn update(&mut self, interface: &mut dyn Interface) -> UnitControlResult {
        ...
        let team = interface.get_i32(common::MODULE_TEAM, REG_TEAM_TEAMID)?;

        let count = interface.get_i32(
            common::MODULE_OBJECTIVES,
            REG_OBJECTIVES_CAPTURE_POINT_COUNT,
        )?;
        for i in 0..count as u32 {
            let x = interface.get_f32(
                common::MODULE_OBJECTIVES,
                REG_OBJECTIVES_CAPTURE_POINT_START
                    + (i * REG_OBJECTIVES_CAPTURE_POINT_STRIDE)
                    + REG_OBJECTIVES_CAPTURE_POINT_OFFSET_X,
            )?;
            let y = interface.get_f32(
                common::MODULE_OBJECTIVES,
                REG_OBJECTIVES_CAPTURE_POINT_START
                    + (i * REG_OBJECTIVES_CAPTURE_POINT_STRIDE)
                    + REG_OBJECTIVES_CAPTURE_POINT_OFFSET_Y,
            )?;
            let owner = interface.get_i32(
                common::MODULE_OBJECTIVES,
                REG_OBJECTIVES_CAPTURE_POINT_START
                    + (i * REG_OBJECTIVES_CAPTURE_POINT_STRIDE)
                    + REG_OBJECTIVES_CAPTURE_POINT_OFFSET_OWNER,
            )?;
            let radius = interface.get_f32(
                common::MODULE_OBJECTIVES,
                REG_OBJECTIVES_CAPTURE_POINT_START
                    + (i * REG_OBJECTIVES_CAPTURE_POINT_STRIDE)
                    + REG_OBJECTIVES_CAPTURE_POINT_OFFSET_RADIUS,
            )?;
