...
impl UnitControl for DiffDriveForwardsBackwardsControl {
    fn update(&mut self, interface: &mut dyn Interface) {
        ...
        // base
        interface
            .set_f32(tank::MODULE_DIFF_DRIVE, 2, self.velocities.0)
            .unwrap();
        interface
            .set_f32(tank::MODULE_DIFF_DRIVE, 3, self.velocities.1)
            .unwrap();
        interface
            .set_f32(tank::MODULE_REVOLUTE_TURRET, revolve_cmd_vel, 3.0)
            .unwrap();
        // interface.set_f32(tank::MODULE_REVOLUTE_BARREL, revolve_cmd_vel, 3.0).unwrap();
    }
}
