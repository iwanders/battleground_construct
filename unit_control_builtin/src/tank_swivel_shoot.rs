...
impl UnitControl for TankSwivelShoot {
    fn update(&mut self, interface: &mut dyn Interface) {
        ...
        let write_res = interface.set_i32(tank::MODULE_CANNON, REG_CANNON_FIRING, true as i32);
        write_res.unwrap();

        let clock = interface.get_f32(tank::MODULE_CLOCK, 0).unwrap();
        if clock < 0.1 {
            interface
                .set_f32(tank::MODULE_REVOLUTE_TURRET, REG_REVOLUTE_VELOCITY_CMD, 0.3)
                .unwrap();
            interface
                .set_f32(
            ...
            || (turret_pos > PI / 8.0 && turret_pos < PI)
        {
            interface
                .set_f32(
                    ...
                    REG_REVOLUTE_VELOCITY_CMD,
                    -interface
                        .get_f32(tank::MODULE_REVOLUTE_TURRET, REG_REVOLUTE_VELOCITY)
                        .unwrap(),
                )
                .unwrap();
        }
        ...
        if barrel_pos < (PI * 2.0 - PI / 8.0) || (barrel_pos < PI / 8.0) {
            interface
                .set_f32(
                    ...
                    REG_REVOLUTE_VELOCITY_CMD,
                    -interface
                        .get_f32(tank::MODULE_REVOLUTE_BARREL, REG_REVOLUTE_VELOCITY)
                        .unwrap(),
                )
                .unwrap();
        }
