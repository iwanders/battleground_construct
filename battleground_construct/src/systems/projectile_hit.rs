...
impl System for ProjectileHit {
    fn update(&mut self, world: &mut World) {
        ...
        let mut projectiles = world.component_iter::<PointProjectile>().map(|(entity, projectile)|{ (entity, projectile.source()) }).collect::<Vec<(EntityId, EntityId)>>();
        let projectile_poses = projectiles.drain(..).map(|(projectile_id, source_id)| {
...

        {
            ...
            let mut hitspheres = world.component_iter::<HitSphere>();
            let hit_sphere_with_pose = hitspheres.map(|(entity, sphere)| {
            ...
            for (projectile_entity, source_id, projectile_pose) in projectile_poses.iter() {
                for (sphere_entity, sphere_pose, sphere) in hit_sphere_with_pose.iter() {
                    ...
                    if inside {
                        ...
                        hit_projectile_source.push((*sphere_entity, *projectile_entity, *source_id));
                    }
                }
            }
        }
        ...
        for (hit_sphere_entity, projectile_entity, source_entity) in hit_projectile_source {
            world.add_component(&hit_sphere_entity, HitBy::new(projectile_entity, source_entity));
        }
    }
}
