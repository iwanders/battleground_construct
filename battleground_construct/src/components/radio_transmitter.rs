...
impl RadioTransmitter {
    ...
    pub fn to_transmit(&mut self, current_time: f32) -> Vec<Vec<u8>> {
        ...
        while !self.messages.is_empty() && self.next_send_time < current_time {
    ...
    pub fn set_messages(&mut self, messages: &[&[u8]]) {
        self.messages = messages
            .iter()
            .take(self.config.message_count_limit)
            .map(|v| v[0..v.len().min(self.config.message_size_limit)].to_vec())
            .collect();
    }
    ...
    fn get_registers(&self, world: &World, registers: &mut RegisterMap) {
        ...
        if let Some(radio_transmitter) = world.component::<RadioTransmitter>(self.entity) {
            ...
            registers.insert(
                ...
                Register::new_i32(
                    "message_size_limit",
                    radio_transmitter.config.message_size_limit as i32,
                ),
            );
            registers.insert(
                ...
                Register::new_i32(
                    "message_count_limit",
                    radio_transmitter.config.message_count_limit as i32,
                ),
            );
            registers.insert(
            ...
            let messages = radio_transmitter.messages();
            registers.insert(
                REG_RADIO_TX_MSG_COUNT,
                Register::new_i32("payload_count", messages.len() as i32),
            );
    ...
    fn set_component(&self, world: &mut World, registers: &RegisterMap) {
        if let Some(mut radio_transmitter) = world.component_mut::<RadioTransmitter>(self.entity) {
            ...
            let messages = (0..upper)
                ...
                .collect::<Vec<&[u8]>>();
            radio_transmitter.set_messages(&messages);
        }
    }
}
