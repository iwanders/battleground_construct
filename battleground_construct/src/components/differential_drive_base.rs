...
impl UnitModule for DifferentialDriveBaseModule {
    fn get_registers(&self, world: &World, registers: &mut RegisterMap) {
        ...
        if let Some(base) = world.component::<DifferentialDriveBase>(self.entity) {
            ...
            registers.insert(REG_LEFT_VEL, Register::new_f32("left_wheel_vel", vels.0));
            registers.insert(REG_RIGHT_VEL, Register::new_f32("right_wheel_vel", vels.1));

            // commanded is the same as reported atm.
            registers.insert(REG_LEFT_CMD, Register::new_f32("left_wheel_cmd", vels.0));
            registers.insert(REG_RIGHT_CMD, Register::new_f32("right_wheel_cmd", vels.1));
        }
    }
