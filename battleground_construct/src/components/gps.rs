...
impl UnitModule for GpsModule {
    fn get_registers(&self, world: &World, registers: &mut RegisterMap) {
        ...
        registers.insert(REG_X, Register::new_f32("x", translation.x));
        registers.insert(REG_Y, Register::new_f32("y", translation.y));
        registers.insert(REG_Z, Register::new_f32("z", translation.z));

        let rpy = pose.to_rpy();
        registers.insert(REG_ROLL, Register::new_f32("r", rpy.x));
        registers.insert(REG_PITCH, Register::new_f32("p", rpy.y));
        registers.insert(REG_YAW, Register::new_f32("y", rpy.z));
    }
}
