...
fn test_ball()
{
    ...
    {
        let pose = world.component::<Pose>(&ball).expect("Should have a pose for ball");
    ...
    {
        let mut vel = world.component_mut::<Velocity>(&ball).expect("Should have a velocity for ball");
        vel.v[0] = 1.0;
        vel.v[2] = 1.0;
    }
    // Add gravity.
    {
        let mut vel = world.component_mut::<Acceleration>(&ball).expect("Should have a acceleration for ball");
        vel.dv[2] = -9.81;
    }
    ...
    for i in 0..20 {
        ...
        systems.update(&mut world);
        let pose = world.component::<Pose>(&ball).expect("Should have a pose for ball");
        let vel = world.component::<Velocity>(&ball).expect("Should have a vel for ball");
        let accel = world.component::<Acceleration>(&ball).expect("Should have a accel for ball");
